...
export default function TeacherStudentsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Define a type for the course data from API
        type CourseFromAPI = Omit<Course, 'studentsEnrolled'> & {
          studentsEnrolled?: (string | User)[];
        };

        // Normalize studentsEnrolled to string[] to match Course type
        const normalizedCourses: Course[] = teacherCourses.map((course: CourseFromAPI) => ({
          ...course,
          studentsEnrolled: (course.studentsEnrolled || []).map((student: string | User) =>
          ...
          if (course.studentsEnrolled && course.studentsEnrolled.length > 0) {
            for (const studentEntry of course.studentsEnrolled) {
              ...
              if (!studentMap.has(studentId)) {
                // Create ExtendedUser object based on the type of studentEntry
                if (typeof studentEntry === "string") {
                  // If it's just an ID, create a placeholder student
                  ...
                  // If it's a User object, create ExtendedUser from it
                  const extendedStudent: ExtendedUser = {
                    ...studentEntry,
                    _id: studentId,
                    name: `${studentEntry.firstName || ""} ${studentEntry.lastName || ""}`.trim() || "Unnamed Student",
                    email: studentEntry.email || "email@example.com",
                  };
  ...
  const filteredStudents = students.filter((student: ExtendedUser) => {
    // Ensure search matches name or email
    const matchesSearch =
      searchQuery === "" ||
    ...
    const matchesCourse =
      currentCourse === "all" ||
      courses.some((course) => {
        // Check if student ID exists in studentsEnrolled array
        return course._id === currentCourse && 
               course.studentsEnrolled.some(enrolledId => enrolledId === student._id);
      });

    // Filter by enrollment status (mocked as all students are active for now)
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredStudents.length > 0 ? (
                filteredStudents.map((student: ExtendedUser) => {
